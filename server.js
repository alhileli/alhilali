...
async function getPortfolioDataAndLog() {
    ...
    if (openPositions.length === 0) {
        aiState.mood = 'Sleeping';
        aiState.message = 'لا توجد مهمات نشطة. في وضع الاستعداد.';
    } else if (overallPnl > 10) {
        aiState.mood = 'Celebrating';
        aiState.targetSelector = '#best-trades-body';
        aiState.message = `إنجاز رائع أيها القائد! الأرباح تتجاوز التوقعات!`;
    } else if (overallPnl > 0.5) {
        aiState.mood = 'Happy';
        aiState.targetSelector = '#positions-table';
        aiState.message = 'تم رصد إشارات إيجابية. المهمات تسير بنجاح.';
    } else if (overallPnl < -10) {
        aiState.mood = 'Sad';
        aiState.targetSelector = '#worst-trades-body';
        aiState.message = 'تنبيه! تم رصد خسائر كبيرة. يوصى بمراجعة الاستراتيجية.';
    } else if (overallPnl < -0.5) {
        aiState.mood = 'Worried';
        aiState.targetSelector = '#positions-table';
    ...
    return {
        ...
        openPositions: fullProcessedPositions,
    ...
    try {
        const history = await pool.query(`
            WITH daily_equity AS (
                ...
                FROM portfolio_history WHERE timestamp > NOW() - INTERVAL '90 days' GROUP BY day
